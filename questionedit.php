			...
			$id=$_REQUEST["id"];
			 $con=mysql_connect("localhost","root","");
						...
						while($row=mysql_fetch_array($res,MYSQL_ASSOC))
                        {
							$qid=$row["question_id"];
							$que=$row["question"];
							$que_status=$row["question_status"];
							$ans=$row["answer"];
							$eid=$row["email_id"];
						}
			?>
            <form method="post" action="edit_question.php?id=<?php echo $qid;?>" role="form">
			
              <div class="box-body">
			  <div class="form-group" hidden>
                  <label>Question ID</label>
                  <input type="text" disabled class="form-control" name="txt_queid" value="<?php echo $qid;?>">
                </div>
                <div class="form-group">
                  <label>Edit Question Name</label>
                  <input type="text" class="form-control" name="txt_quename" value="<?php echo $que;?>">
                </div>
				<div class="form-group">
                  <label>Select Status</label>
                  <select class="form-control" name="questat">
                    <option value="accept"<?php if($que_status=="accept") echo 'selected';?>>accept</option>
                    <option value="reject"<?php if($que_status=="reject") echo 'selected';?>>reject</option>
                  </select>
                </div>
                
				<div class="form-group">
                  <label>Answer</label>
				<textarea class="form-control" name="que_ans" cols="5" rows="5" required><?php echo $ans;?></textarea>
				</div>
				
				<div class="form-group" disabled>
                  <label>User Email ID</label>
                  <input type="text" disabled class="form-control" name="txt_queid" value="<?php echo $eid;?>">
                </div>
